...
function createLeave(users, applicantEmail){
  ...
  const hods = users.filter(u => (u.role || '').includes('HOD') && related.includes(u.department));
  const viewers = hods.map(h=>h.email);
  const primary = users.find(u => (u.role || '').includes('HOD') && u.department === dept);
...
function simulate(){
  ...
  const applicant = users.find(u=>u.department==='Finance' && !(u.role || '').includes('HOD'));
